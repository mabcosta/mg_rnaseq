...
# Parse parameters
while getopts 'n:d:o:x:r:f:g:s:' OPTION; do
	case "$OPTION" in
		n)
...
# Variables
project_name="test"  # Keep this the same so that of results are found in outdir/project_name/output_per_sample/
input_raw_data_dir="/media/pontikos_nas2/MarcosCosta/projects/MIR184/MIR184_RNAseq/raw_data" # Source directory with all your FASTQ/FQ.gz samples. Add '/*' if you have sample sub-dirs as input_dur/raw_data/*
outdir="."
workdir="."

## If using a non-human/custom reference, add full file paths :
custom_reference="n" # y/n 
fasta_file="."
gtf_file="."
...
prepare_samplesheet () {
	...
	fq_files="$input_raw_data_dir/*q.gz"
	
	total_samples_found=$(basename -a $fq_files | cut -d "_" -f 1 | sort -u | wc -l)
	echo "Uniquely named samples found on raw data source >> $(tput setaf 2)$total_samples_found$(tput sgr0) <<"
	
	# Get full
	ls $fq_files > paths.tmp
	
	# Read the file line by line, store odd lines, and append even lines with comma
	read_1=""
	while read -r read_2; do
			if [ -z "$read_1" ]; then
					read_1="$read_2"
			else
					echo "$read_1,$read_2"
	...
	done < paths.tmp > read_pair.tmp

	# If there's an odd line left at the end, print it without a comma
	if [ -n "$read_1" ]; then
		echo "$read_1"
	fi

	# Add sample name and auto
	echo "sample,fastq_1,fastq_2,strandedness" > samplesheet.csv

	while read -r pair; do
			sample=$(echo $pair | rev | cut -d"/" -f1 | rev | cut -d"_" -f1 )
			echo "$sample,$pair,auto"
	done < read_pair.tmp | sort >> samplesheet.csv
	
	samplesheet_samples=$(tail -n+2 samplesheet.csv | cut -d , -f 1 | sort | uniq | wc -l )
	echo "Uniquely named samples found on samplesheet.csv >> $(tput setaf 2)$samplesheet_samples$(tput sgr0) <<"

	if [[ $samplesheet_samples != $total_samples_found ]]; then
		echo "Not all samples were correctly added to the sample sheet!"
		else
		echo "$(tput setaf 69)Samplesheet made successfully$(tput sgr0)"
	fi
	
	rm paths.tmp read_pair.tmp
}
